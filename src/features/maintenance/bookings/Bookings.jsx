...
const updateStatus = async (bookingId, newStatusString) => {
  ...
  const getProblemDisplay = (booking) => {
    ...
    // لو المشكلة string عادي
    if (typeof booking.problem === "string" && booking.problem.trim() !== "") {
      return booking.problem.trim();
    }

    // لو موجودة داخل object
    if (booking.problem && typeof booking.problem === "object") {
      return (
        booking.problem.description ||
        booking.problem.title ||
        booking.problem.details ||
        booking.problem.name ||
        "—"
      );
    }

    // لو في حقل تاني ممكن يحتوي على وصف المشكلة
