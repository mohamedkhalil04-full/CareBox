...
const InvoiceEditSection = ({ requestId, toast }) => {
  ...
  // ── ADD — uses the invoice's own ID (invoice.invoiceId) as bookingId.
  //         The emergency requestId is NOT the bookingId the endpoint expects;
  //         the backend needs the actual invoice record ID (e.g. 868).
  //         After success, state is updated optimistically so the total
  //         recalculates immediately without a full re-fetch.
  const handleAddItem = async () => {
    ...
    // Resolve the parent invoice ID — the key the endpoint requires as bookingId.
    const invoiceId = invoice?.invoiceId ?? invoice?.id ?? null;
    if (!invoiceId) {
      toast.push("Cannot add item: invoice ID not found. Try refreshing.", "danger");
      return;
    }
    ...
    try {
      const res = await api.put(
        "/Invoices/AddCustomItemsToInvoice",
        { bookingId: invoiceId, items: [{ itemDescription: desc, price }] },
        { headers: { "Content-Type": "application/json" } },
      );

      // The API may return the newly created item with its server-assigned ID.
      // If it does, use that ID so Edit/Delete work immediately on the new row.
      // Otherwise fall back to a re-fetch to get the real invoiceDetailId.
      const responseData = res.data?.data ?? res.data ?? null;
      const returnedItems = Array.isArray(responseData?.items)
        ? responseData.items
      ...
      if (returnedItems && returnedItems.length > 0) {
        // Merge returned items (which carry real invoiceDetailIds) into state.
        setItems(prev => [...prev, ...normaliseItems(returnedItems)]);
      } else {
        // API didn't return item details — re-fetch to get the real IDs.
        await fetchInvoice();
      }
