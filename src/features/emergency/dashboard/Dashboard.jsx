...
const EmergencyDashboard = () => {
  ...
  // ── Date helpers ────────────────────────────────────────────────────────
  // Compare only the calendar date (year/month/day) so timezone-safe matching.
  const toDateStr = (d) => {
    const dt = new Date(d);
  ...
  const isToday     = (r) => r.createdAt && toDateStr(r.createdAt) === todayStr;
  const isYesterday = (r) => r.createdAt && toDateStr(r.createdAt) === yesterdayStr;

  // ── Card 1: Today's Requests vs Yesterday ───────────────────────────────
  // Main value still uses stats.todayRequestsCount (server-provided).
  // Badge: compare it against client-computed yesterday count from raw array.
  const yesterdayTotalCount = requests.filter(isYesterday).length;
  const todayVsYesterdayDiff = (stats.todayRequestsCount ?? 0) - yesterdayTotalCount;
  const card1Badge = (() => {
    if (yesterdayTotalCount === 0 && (stats.todayRequestsCount ?? 0) === 0) {
      return { text: "No data yet", icon: "fa-solid fa-minus", cls: "text-muted" };
    }
    if (yesterdayTotalCount === 0) {
      return { text: `${stats.todayRequestsCount} today`, icon: "fa-solid fa-arrow-trend-up", cls: "text-success" };
    }
    const sign = todayVsYesterdayDiff >= 0 ? "+" : "";
    const cls  = todayVsYesterdayDiff > 0 ? "text-success" : todayVsYesterdayDiff < 0 ? "text-danger" : "text-muted";
    const icon = todayVsYesterdayDiff > 0 ? "fa-solid fa-arrow-trend-up"
               : todayVsYesterdayDiff < 0 ? "fa-solid fa-arrow-trend-down"
               : "fa-solid fa-minus";
    return { text: `${sign}${todayVsYesterdayDiff} vs yesterday`, icon, cls };
  })();

  // ── Card 3: Completed TODAY vs Completed YESTERDAY ──────────────────────
  const completedTodayCount     = requests.filter(r => isToday(r)     && normaliseStatus(r.status) === 5).length;
  const completedYesterdayCount = requests.filter(r => isYesterday(r) && normaliseStatus(r.status) === 5).length;
  const completedDiff = completedTodayCount - completedYesterdayCount;
  const card3Badge = (() => {
    if (completedYesterdayCount === 0 && completedTodayCount === 0) {
      return { text: "None today", icon: "fa-solid fa-minus", cls: "text-muted" };
    }
    if (completedYesterdayCount === 0) {
      return { text: `${completedTodayCount} completed`, icon: "fa-solid fa-arrow-trend-up", cls: "text-success" };
    }
    const sign = completedDiff >= 0 ? "+" : "";
    const cls  = completedDiff > 0 ? "text-success" : completedDiff < 0 ? "text-danger" : "text-muted";
    const icon = completedDiff > 0 ? "fa-solid fa-arrow-trend-up"
               : completedDiff < 0 ? "fa-solid fa-arrow-trend-down"
               : "fa-solid fa-minus";
    return { text: `${sign}${completedDiff} vs yesterday`, icon, cls };
  })();
    ...
    <div className="container-fluid bg-light min-vh-100">
      ...
      <div className="row g-3 mb-4">
        <StatCard
          ...
          badge={card1Badge.text}
          badgeIcon={card1Badge.icon}
          badgeClass={card1Badge.cls}
        ...
        <StatCard
          ...
          badge={card3Badge.text}
          badgeIcon={card3Badge.icon}
          badgeClass={card3Badge.cls}
