...
const SparePartsHome = () => {
  ...
  // Single source of truth — the full raw orders array from the API.
  // ALL card values, pie chart, and month comparisons are derived from this.
  const [allOrders, setAllOrders]     = useState([]);
  const [monthlyData, setMonthlyData] = useState([]);
  const [earningsYear, setEarningsYear] = useState(new Date().getFullYear());

  // ── Fetch: all orders (single call powers cards + pie + table) ──────────
  ...
  // ── Fetch: monthly earnings for the area chart (unchanged) ──────────────
  const fetchMonthlyEarnings = useCallback(async (year) => {
    setMonthlyLoading(true);
    try {
      const res  = await api.get(`/Dashboard/ProviderDashboard/MonthlyEarnings?year=${year}`);
  ...
  useEffect(() => { fetchMonthlyEarnings(earningsYear); }, [earningsYear, fetchMonthlyEarnings]);

  // ── Client-side month helpers ────────────────────────────────────────────
  const now       = new Date();
  ...
  const inMonth = (order, y, m) => {
    if (!order.orderDate) return false;
    const d = new Date(order.orderDate);
    return d.getFullYear() === y && d.getMonth() === m;
  };

  const currentMonthOrders = allOrders.filter(o => inMonth(o, curYear,  curMonth));
  const lastMonthOrders    = allOrders.filter(o => inMonth(o, prevYear, prevMonth));

  // ── pctChange: safe % change, returns null when previous = 0 ────────────
  const pctChange = (cur, prev) => {
    if (prev === 0) return cur === 0 ? null : null; // no meaningful baseline
  ...
  const trendTotalOrders = pctChange(curTotalOrders, prevTotalOrders);

  // ── Card 2: Total Income (sum of totalPrice) ─────────────────────────────
  const sumIncome = (orders) => orders.reduce((s, o) => s + Number(o.totalPrice ?? 0), 0);
  const curIncome   = sumIncome(currentMonthOrders);
  const prevIncome  = sumIncome(lastMonthOrders);
  const trendIncome = pctChange(curIncome, prevIncome);

  // ── Card 3: Cancelled Orders ─────────────────────────────────────────────
  const isCancelled = (o) => (o.statusName ?? "").toLowerCase() === "cancelled";
  const curCancelled  = currentMonthOrders.filter(isCancelled).length;
  const prevCancelled = lastMonthOrders.filter(isCancelled).length;
  const trendCancelled = pctChange(curCancelled, prevCancelled);

  // ── Card 4: Pending Orders (current month only, no comparison shown) ─────
  const isPending = (o) => (o.statusName ?? "").toLowerCase() === "pending";
  const curPending = currentMonthOrders.filter(isPending).length;

  // ── Pie: distribution of current month's orders by status ────────────────
  const STATUS_KEYS = Object.keys(STATUS_COLORS); // canonical capitalised keys
  const pieData = STATUS_KEYS
    .map(key => ({
      name:   STATUS_LABELS[key] ?? key,
      rawKey: key,
      value:  currentMonthOrders.filter(
                o => (o.statusName ?? "").toLowerCase() === key.toLowerCase()
              ).length,
    }))
    .filter(e => e.value > 0);
  ...
  // ── Recent orders: last 5 from full list ────────────────────────────────
  const recentOrders = allOrders.slice(0, 5);

  // statsLoading mirrors ordersLoading for the cards
  ...
  return (
    <div className="p-2 p-lg-2" style={{ background: "#f8fafc", minHeight: "100vh" }}>
      ...
      {/* ── Stat Cards — all values derived from currentMonthOrders ── */}
      <div className="row g-4 mb-5">
        <StatCard
          icon="fas fa-shopping-bag"
          label="Total Orders"
          value={curTotalOrders.toLocaleString()}
          trend={trendTotalOrders}
          trendLabel={trendTotalOrders !== null ? "vs last month" : "vs last month"}
          iconColor="#3b82f6"
          loading={statsLoading}
        />
        <StatCard
          icon="fas fa-wallet"
          label="Total Income"
          value={`${curIncome.toLocaleString()} EGP`}
          trend={trendIncome}
          trendLabel={trendIncome !== null ? "vs last month" : "vs last month"}
          iconColor="#22c55e"
          loading={statsLoading}
        />
        <StatCard
          icon="fas fa-times-circle"
          label="Cancelled Orders"
          value={curCancelled.toLocaleString()}
          trend={trendCancelled}
          trendLabel={trendCancelled !== null ? "vs last month" : "vs last month"}
          iconColor="#ef4444"
          loading={statsLoading}
        />
        <StatCard
          icon="fas fa-clock"
          label="Pending Orders"
          value={curPending.toLocaleString()}
          trendLabel="⚡ Requires action"
          iconColor="#f59e0b"
          loading={statsLoading}
        />
