...
───────────────────────────────────────────── */
const SPARE_PARTS = [
  "Brake Pads", "Oil Filter", "Spark Plugs", "Air Filter",
  "Headlight Bulb", "Shock Absorber", "Timing Belt", "Alternator",
  "Radiator", "Fuel Pump", "Clutch Kit", "Water Pump",
  "Cabin Air Filter", "Battery", "Starter Motor", "Catalytic Converter",
  "CV Joint", "Wheel Bearing", "Tie Rod End", "Serpentine Belt",
];

const BRAND_MODELS = {
  Toyota: ["Corolla", "Camry", "RAV4", "Land Cruiser", "Yaris", "Hilux", "Prado"],
  BMW:    ["3 Series", "5 Series", "7 Series", "X5", "X3", "M4", "M5", "i4", "i8"],
  Audi:   ["A4", "A6", "A8", "Q5", "Q7", "Q8", "e-tron", "RS6", "TT"],
  Mercedes: ["C-Class", "E-Class", "S-Class", "GLE", "GLC", "A-Class", "CLA"],
  Hyundai: ["Elantra", "Tucson", "Santa Fe", "i10", "i20", "Accent", "Sonata"],
  Kia:     ["Cerato", "Sportage", "Sorento", "Picanto", "K5", "Stinger"],
  Chevrolet: ["Cruze", "Malibu", "Spark", "Captiva", "Equinox", "Tahoe"],
  Ford:    ["F-150", "Escape", "Explorer", "Focus", "Fusion", "Mustang"],
  Nissan:  ["Altima", "Sentra", "X-Trail", "Patrol", "Navara", "Sunny"],
...
const SparePartsProducts = () => {
  ...
  const handleChange = (e) => {
    const { name, value } = e.target;
    // When brand changes, reset model to prevent invalid brand/model combos
    if (name === "make") {
      setFormData(prev => ({ ...prev, make: value, forModel: "" }));
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingProductId) {
        // UPDATE (PUT) — multipart/form-data as confirmed by the API docs
        await api.put(`/Products/Update-product/${editingProductId}`, data, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        ...
        setProducts(prev => prev.map(p =>
          p.productId === editingProductId
            ? {
                ...
                make:               formData.make,
                forModel:           formData.forModel,
                year:               formData.year,
                condition:          formData.condition === "1" ? "New"
                                  : formData.condition === "2" ? "Used"
                                  : "Refurbished",
                horizontalPosition: formData.horizontalPosition,
                verticalPosition:   formData.verticalPosition,
                categoryName:       formData.categoryName,
                // keep existing images unless a new file was uploaded
                ...(imageFile && previewUrl ? { imageUrl: previewUrl } : {}),
              }
            : p
        ));
        alert("✅ Product updated successfully!");
        ...
        alert("✅ Product added successfully!");
        fetchProducts(); // re-fetch to get server-assigned productId for new items
      }
  ...
  return (
    <div className="p-2 p-lg-2 bg-light min-vh-100">
      {/* Header */}
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="mb-0 fw-bold">Products</h2>
        <button
                ...
                <Form.Group>
                  ...
                  {!formData.make && (
                    <Form.Text className="text-muted" style={{ fontSize: "0.78rem" }}>
                      Choose a brand above to unlock model options.
                    </Form.Text>
                  )}
                </Form.Group>
              </Col>
